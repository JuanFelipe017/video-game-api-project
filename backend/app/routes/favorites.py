...
from app.models.schemas import FavoriteCreate, FavoriteDelete
...
@router.delete("/")
def delete_favorite(data: FavoriteDelete, current_user: int = Depends(verify_user)):
    result = favorites_controller.remove_favorite(data.user_id, data.game_id)
    if "error" in result:
